...
function generatePassword() {
    ...
    if (lowerCase == false && upperCase == false && number == false && speChar == false) {
        ...
        alert("Your password contains lowercase letters only");
        //return random lowercase letters only within the input lenght
        var lowerCasePw = "";
        var possible = "abcdefghijklmnopqrstuvwxyz";

        function returnLowercasePw() {
            for (var i = 0; i <= pwLength; i++) {
                lowerCasePw += possible.charAt(Math.floor(Math.random() * possible.length));
            }
            return lowerCasePw;
        }
        returnLowercasePw()
